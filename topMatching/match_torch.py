...
def train_epoch_batch(model, opt, criterion, batch_size=200):
    model.train()
    #losses = []
    ...
    for beg_i in range(0, X.size(0), batch_size):
        ...
        loss = criterion(y_hat_batch.float(), y_batch.float())
        # (3) Compute gradients
        loss.backward()
        # (4) update weights
        opt.step()        
...
def train_epoch(model, opt, criterion, batch_size=200):
    model.train()
    #losses = []
    opt.zero_grad()
    # (1) Forward
...
for p in param_grid:
    ...
    for e in range(p.epochs):
        ...
        test_loss = criterion(y_pred_test.float(), Y_test.float()).float().detach().numpy()

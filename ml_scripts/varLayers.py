...
class Net(nn.Module):
    ...
    def forward(self, input_):
        ...
        h1 = self.dout(self.relu1(self.f125t125(h1)))
        h1 = self.dout(self.relu1(self.f125t125(h1)))
        h1 = self.dout(self.relu1(self.f125t25(h1)))
        #h1 = self.dout(self.relu1(self.f75t75(h1)))
        #h1 = self.dout(self.relu1(self.f75t75(h1)))
        #h1 = self.dout(self.relu1(self.f75t25(h1)))
...
lnPairs = [[9,150]]#[[5,100],[6,75],[6,100],[8,50],[8,75]]
...
for p in param_grid:
    ...
    torch.save(net.state_dict(), 'torch_models/'+outDir+'/model_varLayers.pt')
    torch.save(p.y_pred_test, 'torch_models/'+outDir+'/y_test_pred_varLayers.pt')
    torch.save(p.y_pred, 'torch_models/'+outDir+'/y_train_pred_varLayers.pt')
    ...
    plt.savefig('plots/'+outDir+'/torch_loss_varLayers.png')
    ...
    plt.ylabel('NEvents')
    plt.legend(loc='upper right')
    plt.savefig('plots/'+outDir+'/torch_train_pt_spectrum_varLayers.png')
    ...
    plt.ylabel('NEvents')
    plt.legend(loc='upper right')
    plt.savefig('plots/'+outDir+'/torch_test_pt_spectrum_varLayers.png')
    ...
    plt.savefig('plots/'+outDir+'/torch_roc_varLayers.png')
    '''
    plt.figure()
    for c in cutoff:
        yTest = np.where(Y_full_test > c, 1, 0)
        ...
        plt.legend(loc='lower right')
    plt.savefig('plots/'+outDir+'/torch_test_roc_varLayers.png')
    '''
    ...
    plt.savefig('plots/'+outDir+'/torch_test_pt_scatter_varLayers.png')
    ...
    plt.ylabel('Predicted Pt')
    plt.plot([0,800000],[0,800000], zorder=10)
    plt.savefig('plots/'+outDir+'/torch_train_pt_scatter_varLayers.png')

...
def addToFile(outPath, y, name, treeName):
    with root_open(outPath, mode='a') as myfile:
        y = np.asarray(y)
        y.dtype = [(name, 'float32')]
        y.dtype.names = [name]
        root_numpy.array2tree(y, tree=myfile.Get(treeName))
        myfile.write()
        myfile.Close()
...
def addPred(inf):
    ...
    for tree in get_tree_list(inf):
        print(f'On {tree} in {inf}')

        outReco = runReco(inf, tree)
        if not outReco:
            continue
        if channel=='2lSS':
            ...
            addToFile(inf, higgsRecoScores, "higgsRecoScore2lSS", tree)                                                   
            addToFile(inf, topRecoScores, "topRecoScore", tree) 

            addToFile(inf, pt_pred, 'recoHiggsPt_2lSS', tree)
            addToFile(inf, bin_pred, 'binHiggsPt_2lSS', tree)
            ...
            addToFile(inf, higgsRecoScores, "higgsRecoScore3lS", tree)
            addToFile(inf, higgsRecoScoresF, "higgsRecoScore3lF", tree)
            addToFile(inf, topRecoScores, "topRecoScore", tree)
            
            addToFile(inf, pt_pred, 'recoHiggsPt_3lS', tree)
            addToFile(inf, bin_pred, 'binHiggsPt_3lS', tree)
            
            addToFile(inf, pt_predF, 'recoHiggsPt_3lF', tree)  
            addToFile(inf, bin_predF, 'binHiggsPt_3lF', tree)
            
            addToFile(inf, decay_pred, 'decayScore', tree)

linelist = [line.rstrip() for line in open(sys.argv[1])]
#addPred(linelist[0])
Parallel(n_jobs=6)(delayed(addPred)(inf) for inf in linelist)

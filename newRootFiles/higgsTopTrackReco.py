...
for e in nom:
    ...
    for i in range(len(e.track_jet_pt)):
        ...
        if e.track_jet_parent[i]==25:
            higgsJets.append(i)
        elif abs(e.track_jet_parent[i])==6:
            topJets.append(i)
        else:
            badJets.append(i)
    ...
    k = flatDict( 1, jets[ higgsJets[0] ], jets[ higgsJets[1] ], lepH[0], met, e.track_jet_MV2c10[ higgsJets[0] ], e.track_jet_MV2c10[ higgsJets[0] ], jets[ topJets[0] ], jets[ topJets[1] ] )
    eventsFlat.append(k)

    for l in range(2):
        k = flatDict( 0, jets[ higgsJets[0] ], jets[ higgsJets[1] ], lepB[0], met, e.track_jet_MV2c10[ higgsJets[0] ], e.track_jet_MV2c10[ higgsJets[0] ], jets[ topJets[0] ], jets[ topJets[1] ] )
        eventsFlat.append(k)

    if len(badJets) > 2:
        for l in range(2):
            i,j = random.sample(badJets,2)
            k = flatDict( 0, jets[i], jets[j], lepH[0], met, e.track_jet_MV2c10[i], e.track_jet_MV2c10[j], jets[ topJets[0] ], jets[ topJets[1] ] )
            eventsFlat.append(k)

        for l in range(min([10, len(badJets)])):
            i,j = random.sample(badJets,2)
            k = flatDict( 0, jets[i], jets[j], lepB[0], met, e.track_jet_MV2c10[i], e.track_jet_MV2c10[j], jets[ topJets[0] ], jets[ topJets[1] ] )
            eventsFlat.append(k)
...
dfFlat.to_csv('higgsTopTrackFiles/'+dsid+'Flat.csv', index=False)

...
def make_plots(alg, model, outDir, y_train, y_test, y_train_pred, y_test_pred):
    ...
    plt.figure()
    plt.hist(testPredTrue[:minLen], 30, range=(-0.1,1.1), log=False, alpha=0.5, label='Signal - Test')
    plt.hist(testPredFalse[:minLen], 30, range=(-0.1,1.1), log=False, alpha=0.5, label='Background - Test')
    plt.hist(trainPredTrue[:minLen], 30, range=(-0.1,1.1), log=False, histtype='step', alpha=0.5, label='Signal - Train')
    plt.hist(trainPredFalse[:minLen], 30, range=(-0.1,1.1), log=False, histtype='step', alpha=0.5, label='Background - Train')
